const appsettings = require("utils").App;
    ...
    app.get('/', stormpath.loginRequired, (req, res) => {
        getUserDevices(req.user).then(devices => {
                ...
                for (var i = 0; i < devices.length; i++) {
                    if (req.query.device == devices[i].id) {
                        device = devices[i];
                    }
                }
                if (device == null) {
                ...
                res.render("index.ejs", { user: req.user, devices: devices, device: device, appSettings: appsettings })
            },
            err => {
                console.log(err);
                res.sendStatus(500);
